...
import OracleGlobe from "@/components/OracleGlobe";
    ...
    <Card className="bg-white/5 border-white/10 card-tilt">
      ...
      <CardContent className="grid gap-4 text-sm">
        ...
        <div className="grid gap-2">
          ...
          <Input value={amount} onChange={(e) => setAmount(Number(e.target.value || 0))} type="number" min={0} />
      ...
      <header className="sticky top-0 z-50 backdrop-blur-sm supports-[backdrop-filter]:bg-slate-900/60 border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button asChild className="h-8 px-3 btn-neon">
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-4 text-base md:text-lg">
              “一个生命的诞生 +1，一次离世 -1”。多源预言机驱动供给变动，<span className="text-cyan-300">透明</span>、<span className="text-cyan-300">可验证</span>、与现实世界紧密映射。
            </p>
...
function ExtraStyles() {
  // 内联样式：确保构建不会被 Purge 掉，且不依赖外部 CSS
  return (
    <style>{`
      /* === 全局渐变动画背景 === */
      .gradient-shift {
      ...
      /* === 噪点纹理，提升对比与质感 === */
      .noise {
        background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><filter id="n"><feTurbulence type="fractalNoise" baseFrequency="0.8" numOctaves="2" stitchTiles="stitch"/></filter><rect width="120" height="120" filter="url(#n)" opacity=".035"/></svg>`) }');
        mix-blend-mode: soft-light;
      }

      /* === 标题渐变（Hero） === */
      .title-gradient { 
        background: linear-gradient(90deg,#fff, #9ae6ff 30%, #c7b8ff 60%, #fff); 
      ...
      .btn-neon { 
        ...
        box-shadow: 0 0 32px rgba(34,211,238,.35), inset 0 0 12px rgba(255,255,255,.15); 
        border: none; 
      }
      .btn-neon:hover { 
        filter: brightness(1.05); 
        box-shadow: 0 0 48px rgba(34,211,238,.45), inset 0 0 12px rgba(255,255,255,.25); 
      }
      ...
      /* === ScrollReveal：入场 + 扫描线 === */
      .reveal-section { 
        opacity: 0; transform: translateY(24px); position: relative; 
      }
      ...
      .reveal-section.revealed { 
        opacity: 1; transform: none; transition: opacity .8s ease, transform .8s ease; 
      }
      .reveal-section.revealed::before { 
        animation: scan 1.2s ease 1; 
      }
      @keyframes scan { 
        0% { opacity:.7; transform: translateY(-40%);} 
