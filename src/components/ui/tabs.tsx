  ...
  return (
    <section id={id} className="scroll-mt-24 py-20 reveal-section">
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-8">
          <h2 className="text-2xl md:text-3xl font-semibold tracking-tight title-gradient">
            {title}
          </h2>
          {subtitle && (
  ...
  const ui = (
    <div className="fixed top-16 right-4 space-y-2 z-[60]">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="px-3 py-2 rounded-lg bg-black/60 border border-white/10 shadow-lg text-sm backdrop-blur"
        >
          {t.text}
        </div>
      ))}
...
/* =======================
   Private Sale (UI only)
======================= */
const RATE_RST_PER_USD = 500; // example
  ...
  useEffect(() => {
    ...
    const fetchPrice = async () => {
      try {
        ...
        if (!isNaN(p) && alive) setBnbPrice(p);
      } catch {}
    };
    fetchPrice();
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    outerRadius={110}
                    paddingAngle={2}
                    label={(d: any) => `${Math.round(((d?.percent ?? 0) as number) * 100)}%`}
                    labelLine={false}
                  >
